...
export default function ReleasesPage() {
  ...
  const handleExportPDF = () => {
    ...
    const kpiHtml = '<div class="kpi-row">'
      + '<div class="kpi-card"><div class="kpi-value">' + formatNumber(kpis.total_releases || data?.total || 0) + '</div><div class="kpi-label">Total Releases</div></div>'
      + '<div class="kpi-card"><div class="kpi-value">' + (kpis.avg_days_between_releases != null ? Number(kpis.avg_days_between_releases).toFixed(1) + ' days' : '\u2014') + '</div><div class="kpi-label">Avg Gap</div></div>'
      + '<div class="kpi-card"><div class="kpi-value">' + (kpis.avg_commits_per_release != null ? Number(kpis.avg_commits_per_release).toFixed(1) : '\u2014') + '</div><div class="kpi-label">Avg Commits / Release</div></div>'
      + '<div class="kpi-card"><div class="kpi-value">' + (kpis.last_release_tag || '\u2014') + '</div><div class="kpi-label">Last Release</div></div>'
      + '</div>'

    const releasesTable = buildTable(
      ['Tag', 'Release Name', 'Repo', 'Commits', 'PRs', 'Released At', 'Days Gap'],
      items.map(r => [
        r.tag_name || '\u2014', r.release_name || '\u2014', r.repo_name || '\u2014',
        r.commit_count != null ? formatNumber(r.commit_count) : '\u2014',
        r.pr_count != null ? formatNumber(r.pr_count) : '\u2014',
        fmtDate(r.released_at),
        r.days_since_prev != null ? r.days_since_prev + 'd' : '\u2014',
      ])
    )

    const html = `<!DOCTYPE html><html><head><title>Releases Report</title><style>body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; padding: 24px; color: #1e293b; font-size: 12px; }
      ...
      + '<h1>Releases Report</h1><div class="subtitle">${subtitle}</div>'
      + kpiHtml
      + '<div class="section"><div class="section-title">Releases</div>${releasesTable}</div>'
